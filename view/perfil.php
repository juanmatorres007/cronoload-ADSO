<div class="perfilContent">
  <div>
    <form action="../routes/user.php?action=update">
      ...
      <input type="text" name="phone_id" id="phone" placeholder=""><br><br>

      <label for=""><strong>CORREO ELECTRONICOC: </strong></label>
      <input type="text" name="email_id" id="email" placeholder=""><br><br>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        var id_auto_con = <?php echo $_SESSION['usuario']['id_auto_con']; ?>;

        var xhr = new XMLHttpRequest();
        xhr.open("GET", "getData.php?phone_id=" + id_auto_con, true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
